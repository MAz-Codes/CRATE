...
from model import CrateVAE, generate_square_subsequent_mask, get_token_types, TOKEN_TYPE_MAP
import argparse
import os
import math
...
def train(args):
    ...
    position_criterion = nn.CrossEntropyLoss(ignore_index=-1)  
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (src, src_mask, style_batch, bar_batch) in enumerate(progress):
            ...
            kl_per_dim = torch.mean(kl_element, dim=0)  
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train CrateVAE")
